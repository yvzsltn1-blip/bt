...
(function attachBattleCore(globalScope) {
  ...
  function buildOrders(unitSpeed, unitNumbers = null, unitHealth = null) {
    const FLAGS = globalScope.__SIM_FLAGS__ || {};
    const minSpeed = Math.min(...unitSpeed);
    const maxSpeed = Math.max(...unitSpeed);

    const rearClasses = FLAGS.flipRearOrder
      ? [["enemy", "rear"], ["ally", "rear"]]
      : [["ally", "rear"], ["enemy", "rear"]];
    ...
    for (let speed = maxSpeed; speed >= minSpeed; speed -= 1) {
      if (FLAGS.frontOrder && unitNumbers) {
        for (const [side, position] of rearClasses) {
          for (let j = UNIT_DESC.length - 1; j >= 0; j -= 1) {
            if (unitSpeed[j] === speed && UNIT_DESC[j][SIDE_INDEX] === side && UNIT_DESC[j][POSITION_INDEX] === position) {
              attackerOrder.push(j);
            }
          }
        }
        const fronts = [];
        for (const [side, position] of frontClasses) {
          for (let j = UNIT_DESC.length - 1; j >= 0; j -= 1) {
            if (unitSpeed[j] === speed && UNIT_DESC[j][SIDE_INDEX] === side && UNIT_DESC[j][POSITION_INDEX] === position) {
              fronts.push(j);
            }
        ...
        const key = (j) => {
          if (FLAGS.frontOrder === "countDesc") return unitNumbers[j];
          if (FLAGS.frontOrder === "atkDesc") return unitNumbers[j] * UNIT_DESC[j][ATTACK_INDEX];
          if (FLAGS.frontOrder === "hpDesc") return unitHealth ? unitHealth[j] : 0;
          return 0;
        };
        fronts.sort((a, b) => key(b) - key(a));
        attackerOrder.push(...fronts);
        continue;
      }
      if (FLAGS.frontHpAsc) {
        for (const [side, position] of rearClasses) {
        ...
        const fronts = [];
        for (let j = UNIT_DESC.length - 1; j >= 0; j -= 1) {
          if (unitSpeed[j] === speed && UNIT_DESC[j][POSITION_INDEX] === "front") {
            fronts.push(j);
          }
        }
        const sortHp = (j) => (j === REVIVED_INDEX ? UNIT_DESC[ZOMBIES_INDEX][HEALTH_INDEX] : UNIT_DESC[j][HEALTH_INDEX]);
        fronts.sort((a, b) => {
          const hpDelta = sortHp(a) - sortHp(b);
          if (hpDelta !== 0) return hpDelta;
          const sideRank = (j) => (UNIT_DESC[j][SIDE_INDEX] === "ally" ? 0 : 1);
          return sideRank(a) - sideRank(b);
        });
        attackerOrder.push(...fronts);
        continue;
      }
      for (const [side, position] of classes) {
        for (let j = UNIT_DESC.length - 1; j >= 0; j -= 1) {
          if (unitSpeed[j] === speed && UNIT_DESC[j][SIDE_INDEX] === side && UNIT_DESC[j][POSITION_INDEX] === position) {
            attackerOrder.push(j);
          }
  ...
  function findDefenderForAttacker(attackerIndex, unitNumbers, unitHealth, unitSpeed, defenderOrderFrontFirst, defenderOrderRearFirst, roundCount, excludeIndex = -1) {
    ...
    const FLAGS2 = globalScope.__SIM_FLAGS__ || {};
    const noOccultSkip = excludeIndex === -999;
    if (noOccultSkip) {
      excludeIndex = -1;
    }
    let occultExclude = -1;
    if (!noOccultSkip && FLAGS2.occultSkipRevived && UNIT_DESC[attackerIndex][SIDE_INDEX] === "ally" && UNIT_DESC[attackerIndex][TYPE_INDEX] === "occult") {
      occultExclude = REVIVED_INDEX;
    }
    const prioritizeRear = defenderOrder === defenderOrderRearFirst;
    const softExclude = excludeIndex >= -1 ? false : true; // excludeIndex <= -2 means soft (deprioritize)
    const effectiveExclude = softExclude ? -excludeIndex - 10 : excludeIndex;
    ...
    for (let i = 0; i < defenderOrder.length; i += 1) {
      ...
      if (defenderIndex === effectiveExclude || defenderIndex === occultExclude) {
        continue;
      }
    ...
    if (bestDefenderIndex === -1 && (softExclude || occultExclude !== -1)) {
      return findDefenderForAttacker(attackerIndex, unitNumbers, unitHealth, unitSpeed, defenderOrderFrontFirst, defenderOrderRearFirst, roundCount, -999);
    }
  ...
  function simulateBattle(enemyCounts, allyCounts, options = {}) {
    ...
    let orders = buildOrders(unitSpeed, unitNumbers, unitHealth);
    ...
    while (enemyCapable && allyCapable) {
      ...
      orders = buildOrders(unitSpeed, unitNumbers, unitHealth);
      ...
      for (let j = 0; j < attackerOrder.length; j += 1) {
        ...
        if (foundAttacker) {
          const target = findDefenderForAttacker(
            ...
            defenderOrderRearFirst,
            roundCount,
            FLAGS.reviveUntargetableSpawnRound && revivedSpawnRound === roundCount ? REVIVED_INDEX : FLAGS.reviveDeprioritizedSpawnRound && revivedSpawnRound === roundCount ? -(REVIVED_INDEX + 10) : FLAGS.reviveDeprioritizedAlways ? -(REVIVED_INDEX + 10) : -1
          );
        ...
        if (FLAGS.rotmawDefNeutral && defenderIndex === ROTMAWS_INDEX) {
          damageMultiplier = 1;
        }
        ...
        const bansheeDebuffApplies = FLAGS.bansheeDebuffNextRound
          ? attackerIndex === bansheesReducePrevTarget
          : FLAGS.bansheeDebuffBothRounds
        ...
        const attackerDamage =
          attackerIndex === WITCHES_INDEX && roundCount % 2 === 0
            ? 0
            : FLAGS.perUnitCeil
              ? unitNumbers[attackerIndex] * ceilCombatValue(UNIT_DESC[attackerIndex][ATTACK_INDEX] * damageMultiplier * unitBuffs[attackerIndex])
              : FLAGS.bansheeFloorDamage && attackerIndex === BANSHEES_INDEX
                ? floorCombatValue(rawAttackerDamage)
                : roundFlagVariant(rawAttackerDamage) !== null
                  ? roundFlagVariant(rawAttackerDamage)
        ...
        function roundFlagVariant(raw) {
          ...
          if (FLAGS.allyHalfRandomKeepSpecials && isAlly && isHalf && !(attackerIndex === BATS_INDEX && defenderIndex === GIANTS_INDEX && roundCount === 1)) {
            return rng() < 0.5 ? floorCombatValue(raw) : ceilCombatValue(raw);
          }
          if (FLAGS.bansheeHalfRandom && attackerIndex === BANSHEES_INDEX && isHalf) return rng() < 0.5 ? floorCombatValue(raw) : ceilCombatValue(raw);
          if (FLAGS.fracRandom && frac > 1e-9 && frac < 1 - 1e-9) return rng() < frac ? ceilCombatValue(raw) : floorCombatValue(raw);
          if (FLAGS.bansheeFloorVsCultists && attackerIndex === BANSHEES_INDEX && defenderIndex === CULTISTS_INDEX) return floorCombatValue(raw);
          if (FLAGS.allyFloorVsCultists && isAlly && defenderIndex === CULTISTS_INDEX) return floorCombatValue(raw);
          if (FLAGS.allyHalfDown && isAlly && isHalf) return floorCombatValue(raw);
          if (FLAGS.anyHalfDown && isHalf) return floorCombatValue(raw);
          if (FLAGS.bansheeHalfDown && attackerIndex === BANSHEES_INDEX && isHalf) return floorCombatValue(raw);
          if (FLAGS.allySingleUnitFloor && isAlly && unitNumbers[attackerIndex] === 1) return floorCombatValue(raw);
          if (FLAGS.bansheeSingleFloor && attackerIndex === BANSHEES_INDEX && unitNumbers[attackerIndex] === 1) return floorCombatValue(raw);
          if (FLAGS.allyDisadvFloor && isAlly && damageMultiplier < 1 - 1e-9) return floorCombatValue(raw);
          if (FLAGS.bansheeRearFloor && attackerIndex === BANSHEES_INDEX && UNIT_DESC[defenderIndex][POSITION_INDEX] === "rear") return floorCombatValue(raw);
          if (FLAGS.allyRearFloor && isAlly && UNIT_DESC[defenderIndex][SIDE_INDEX] === "enemy" && UNIT_DESC[defenderIndex][POSITION_INDEX] === "rear") return floorCombatValue(raw);
          return null;
        }
        ...
        if (defenderIndex === REVIVED_INDEX) {
          if (FLAGS.noRecomputeOnZeroDamage && attackerDamage === 0) {
            // 0 hasarli vurus (cadi cift raund) dirilen sayimini tetiklemez.
          } else if (unitHealth[defenderIndex] > 0) {
            const baseHp = FLAGS.revivedCountByOwnHp
              ? UNIT_DESC[REVIVED_INDEX][HEALTH_INDEX]
              : UNIT_DESC[ZOMBIES_INDEX][HEALTH_INDEX];
