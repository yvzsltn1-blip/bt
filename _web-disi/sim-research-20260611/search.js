...
let forceSeedScan = false;
function testRecord(rec, maxSeeds) {
  const seedCount = !forceSeedScan && (rec.enemyCounts.cultists || 0) === 0 ? 1 : maxSeeds;
...
function evaluate(flags, label) {
  setFlags(flags);
  forceSeedScan = Object.keys(flags).some((k) => /Random/.test(k) && flags[k]);
  const fixed = [];
...
// noRecomputeOnZeroDamage uretimde uygulanan duzeltme; her varyantta acik olmali.
const BASE = { noRecomputeOnZeroDamage: true };
const variants = JSON.parse(process.argv[2] || "null") || [
  ["baseline(prod)", {}],
  ["bansheeFloorVsCultists", { bansheeFloorVsCultists: true }],
  ["allyFloorVsCultists", { allyFloorVsCultists: true }],
  ["allyHalfDown", { allyHalfDown: true }],
  ["anyHalfDown", { anyHalfDown: true }],
  ["bansheeHalfDown", { bansheeHalfDown: true }],
  ["allySingleUnitFloor", { allySingleUnitFloor: true }],
  ["bansheeSingleFloor", { bansheeSingleFloor: true }],
  ["allyDisadvFloor", { allyDisadvFloor: true }],
  ["bansheeRearFloor", { bansheeRearFloor: true }],
  ["allyRearFloor", { allyRearFloor: true }],
  ["bansheeFloorDamage(eski)", { bansheeFloorDamage: true }]
];
for (const [label, flags] of variants) evaluate({ ...BASE, ...flags }, label);
